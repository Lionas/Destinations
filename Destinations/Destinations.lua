...
local function FishpinTypeCallback(pinManager)
	...
	if DestinationsSV.filters[DPINS.FISHING_SHOW_BAIT_LEFT] then
		...
		for lureIndex=1, numLures do
			...
			elseif string.find(icon, "fish_tail") and not (string.find(name, "simple") or string.find(name, "einfacher") or string.find(name, "appat")) then	--Chub
			...
			elseif string.find(icon, "fish_tail") and (string.find(name, "simple") or string.find(name, "einfacher") or string.find(name, "appat")) then	--Simle Bait
...
-- Added Error Finder for JP
if(NPCName == nil or NPCName[1] == nil) then
	d("|cFF0000Destinations Error-Finder : NPC ID = " .. NPCID .. "|r")
end
